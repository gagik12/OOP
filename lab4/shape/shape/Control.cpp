...
void CRemoteControl::PrintInfo() const
{
    if (!m_shapes.empty())
    {
        auto figureWithMinPerimeter = std::min_element(m_shapes.begin(), m_shapes.end(), [&](std::shared_ptr<IShape> const& firstShape, std::shared_ptr<IShape> const& secondShape)
        {
            return firstShape->GetPerimeter() < secondShape->GetPerimeter();
        });
        m_output << "\nFigure with minimum perimeter:\n";
        m_output << (*figureWithMinPerimeter)->ToString() << "\n\n";

        auto figureWithMaxArea = std::max_element(m_shapes.begin(), m_shapes.end(), [&](std::shared_ptr<IShape> const& firstShape, std::shared_ptr<IShape> const& secondShape)
        {
            return firstShape->GetArea() < secondShape->GetArea();
        });
        m_output << "\nThe figure with the largest area:\n";
        m_output << (*figureWithMaxArea)->ToString() << "\n\n";
